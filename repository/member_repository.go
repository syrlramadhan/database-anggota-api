...
func (m *memberRepositoryImpl) GetAllMember(ctx context.Context, tx *sql.Tx) ([]model.Member, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&member.IdMember,&member.NRA,&member.Nama,&member.AngkatanID,&member.StatusKeanggotaan,&member.JurusanID,&member.TanggalDikukuhkan,&member.Email,&member.NoHP,&member.Password,&member.Foto,)
		if err != nil {
			return []model.Member{}, err
		}
